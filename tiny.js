...
//
//   For Browsers supports ECMAScript 5+
//	 (IE 8+, Firefox 45+, Chrome 29+, Safari 9.1+)
//
//   Note: Opera Mini don't support window.localStorage
//
//   v1.84 by SiC
//
///////////////////////////////////////////////////////////////////
...
var _tiny = (function () {
	...
	function prepare_tiny() {
		...
		return {
			...
			each: _each,
			CONTINUE: TINY_CONST.CONTINUE,
			BREAK: TINY_CONST.BREAK,
	...
	function _exports() {
		...
		if (_config.registerGlobals) {
			register_globals();
			_warn(CONSOLE_TAG['base'], 'Global functions rigistered.')
		}
	...
	*/
	function register_globals() {
		...
		// prototypes
		String.prototype._htmlSafe = function () {
			return html_safe(this.valueOf());
		};

		String.prototype._formatWith = function (obj) {
		...
		Date.prototype._format = function (format, names) {
			return format_date(this, format, names);
		};
		...
		w._each = _each;
		w._extend = _extend;
		w._namespace = _namespace;

		w._message = _message;
		w._route = _route;

		w._store = local_storage;
		w._cookie = _cookie;

		w._lang = _lang;

		w._format = format_template;
	...
	*/
	 ...
	 * Other console output types can't be disabled
	 * ```
	 *   _tiny.outputLog(true);
	 * ```
	 */
	function output_log(on) {
	 ...
	 * ```
	 *   _each( array_or_object , function(value, index){
	 *     return _tiny.CONTINUE; // to continue
	 *     return _tiny.BREAK;    // to break
	 *   });
	 * ```
	 */
	function _each(obj, func) {
		...
		if (typeof obj !== 'object') {
			_error(CONSOLE_TAG['each'], 'Only Array and Object can be iterated :\n', obj);
			return false;
		}
		if (typeof func !== 'function') {
			_error(CONSOLE_TAG['each'], 'Iteration callback function required :\n', func);
			return false;
		}
	 ...
	 *     == {'old': 200}
	 *   // set true in 3rd parameter to force overwrite
	 *   _extand({'old': 1}, {'old': 200}, true)
	 *     == {'old': 1}
	 * ```
	 */
	function _extend(target, extensions, no_overwrite) {

		// Don't extend non-objects
		if (typeof target != 'object') {
			_error(CONSOLE_TAG['extend'], 'Only Object can be extended :\n', target);
			return target;
		}
		if (typeof extensions != 'object') {
			_error(CONSOLE_TAG['extend'], 'Extension should be an Object :\n', extensions);
			return target;
		}
		...
		_each(extensions, function (item, name) {
			...
			if (no_overwrite && (typeof target[name] !== 'undefined')) {
				return true; // continue
			}
			target[name] = item;
		});
	 ...
	 * ```
	 *    _namespace('my.project', {note: 'this is a container'})
	 *    _namespace('my.project.lib', {nothing: true})
	 * ```
	 */
	function _namespace(namespace_string, obj) {

		if (typeof namespace_string != 'string')
			return null;

		var ns_parts = namespace_string.split('.');
	 ...
	 *     do message_handler('my_message', param1, param2, ...)
	 *        -> return _tiny.BREAK; // to stop message propagation
	 * ```
	 */
	function listen_message(event, handler) {
		...
		_log(CONSOLE_TAG['message.listen'], 'Listen to message: "' + event + '"');
	...
	function post_message(msg) {
		...
		_info(CONSOLE_TAG['message.post'], 'Post message :', msg, sliced_args);

		if (!handles || handles.length < 1) {
			_warn(CONSOLE_TAG['message.post'], 'Nobody is listen to this message');
			return;
		}
		...
		_each(handles, function (callback) {
			count++;
			var result = callback.apply(this, sliced_args);
			if (result === false) return false;
		});

		_log(CONSOLE_TAG['message.post'], count + '/' + handles.length + ' message handlers triggered');
	...
	function check_route(str) {
		...
		_log(CONSOLE_TAG['route.check'], 'Check route: "' + q + '"');
		...
		_each(_route_rules, function (rule, route) {
			...
			if (match) {
				...
				_info(CONSOLE_TAG['route.check'], 'Invoke route rule:', route, q, params);

				// call handlers
				handler_count += _route_handlers[route].length;
		...
		if (!found) {
			_log(CONSOLE_TAG['route.check'], 'No route matched');
		} else {
			_log(CONSOLE_TAG['route.check'], count + '/' + handler_count + ' route handlers called');
		}
	 ...
	 */
	function on_window_hash_change(e) {
		check_route();
	}

	// bind listener immediately
	window.addEventListener('hashchange', on_window_hash_change);


	//////////////////////////////////////////////////////////
	// LOCAL STORAGE & COOKIES FUNCTIONS
	//////////////////////////////////////////////////////////
	var _store = local_storage;
	_store.keyPrefix = ''; // Custom key prefix for data entry filtering
	...
	function local_storage(key, value) {
		...
		if (key === undefined) {
			...
			if (_storage.storageKeyPrefix == '') {
				...
				// the easy way
				_warn(CONSOLE_TAG['store'], 'All items in window.localStorage cleared');
				storage.clear();
				...
				for (var i = 0; i < storage.length; i++) {
					...
					if (key.starsWith(_store.keyPrefix)) {
						storage.removeItem(key);
						i--;
					}
				}

				_warn(CONSOLE_TAG['store'], 'All items in window.localStorage with prefix "' + _store.keyPrefix + '"cleared');
			...
			if (value === undefined) {
				...
				// delete ----------
				_log(CONSOLE_TAG['store'], 'Deleted window.localStorage item: "' + key + '"');
				...
				_log(CONSOLE_TAG['store'], 'Set window.localStorage item: "' + key + '"', value);
	...
	function _cookie(key, value, ttl) {
		...
		if (key === null) {
			...
			_warn(CONSOLE_TAG['cookie'], 'All cookies cleared');
			...
			_warn(CONSOLE_TAG['cookie'], 'Deleted cookie:' + key);
			...
			_log(CONSOLE_TAG['cookie'], 'Set cookie item: "' + key + '"', value);
